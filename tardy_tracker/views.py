...
def home(request):
    if request.user.is_student:
        ...
        # total = CheckIn.objects.filter(course=course).values('student').annotate(dcount=Count('student')).order_by('-dcount')
        total = 0
        if courses:
            has_checked_in = CheckIn.objects.filter(student=request.user, course=courses[0], date=today)
            total = CheckIn.objects.filter(course=courses[0], student=request.user).count()
        # if not total:
        #     total = 0
        data = {
            'courses': courses,
